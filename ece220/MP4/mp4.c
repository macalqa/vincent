...
double abs_double(double y)
{
	...
	result = y;
    return result;                  //finds the absolute value of y, returns result
}
...
double fx_val(double a, double b, double c, double d, double e, double x)
{
	...
	third = (x*x)*c;
	fourth = x*d;
	final = first + second + third + fourth + e;    //finds the function with input x, returns final
...
double fx_dval(double a, double b, double c, double d, double e, double x)
{
	...
	answer = first1 + second1 + third1 + d;
                                                          //finds the derivative of the function at input x, returns answer
    return answer;
}
...
double fx_ddval(double a, double b, double c, double d, double e, double x)
{
	...
	first2 = (x*x)*a*12;
	second2 = 6*b*x;
	answer2 = first2 + second2 + (2*c);               //finds the 2nd derivative at input x, returns answer2
    return answer2;
}
...
double newrfind_halley(double a, double b, double c, double d, double e, double x)
{
    ...
    return (xroot);                                              //calculates the root based off of the Halley's Equation
}                               				// returns the root
...
int rootbound(double a, double b, double c, double d, double e, double r, double l)
{
	...
	result_rootbound = abs_rootbound ;        //calulates the number of possible roots based off of sign change
    						//outputs the result_rootbound, which is the # of roots
...
int main(int argc, char **argv)
{
	...
	if (roots_num == 0)
	{
		...
		return (0);                                                         //if Budan's resulted in 0, there are no roots at the
										  // interval.
	}
	...
	while (l<=r)
		{
			...
			for ((n=0) ; (n <= 10000) ; (n++))
			{
				...
				if (abs_double(x_diff) < 0.000001)
				{
					...
					n = 10001;                              //prints one of the found roots in Halley's method
				}						
  			 x = roots_num;
			...
			if (found_root ==0)
			{
				printf("No roots found. \n");                    // if it didn't enter that loop above, that means
										// there are no roots found; that is printed.
			}	
		l = l + 0.5;	
