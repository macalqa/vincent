...
/* This program assembles partitioned circuit modules into a rectangular chip to
 ...
 functions that this program utilizes, all of which are used to make the floorplan
 desired.*/
node_t* create_child(node_t* par, int n);
...
//This program checks to see if the input node is a leaf node in the tree
//of which it belongs. I checked the left and right pointers to see if they
//were NULL, an indicator of a leaf node.
int is_leaf_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
  if ( ptr->right == NULL && ptr->left == NULL)
  {
    return 1;                       //if right and left pointers of node are NULL, is leaf
  }
  else
  {
    return 0;
...
//This functions checks to see if the node passed in is an internal node.
//I checked to see if the left and right pointers were NULL, and if they weren't
//that means it has children and is therefore and internal node.
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
  if ( ptr->right == NULL && ptr->left == NULL)
  {
    return 0;                  //if right and left pointers are not NULL, is internal node
  }
  else
  {
    return 1;
...
//This function checks to see if the node b is within a subtree of node a.
//I did this by recursively calling the function by passing the parent of b until
//b == NULL or b==a, meaning it either is not in the subtree, or it is in fact,
//within a subtree of "a".
int is_in_subtree(node_t* a, node_t* b)
 {
  // TODO: (remember to modify the return value appropriately)
  if ( a == b)
  {
    return 1;              //b is in subtree of a; base case #1
  }
  if (b == NULL)
  {
    return 0;              //b is not a subtree of a; base case #2
  }
  else
  {
    return is_in_subtree(a , b->parent); //recursive call
  }
}
...
//I completed this function by using the pointers and a temp int in order to swap the values.
void rotate(node_t* ptr) {
  // TODO: Rotate the module, swapping the width and height.
  int temp;
  temp = ptr->module->w;            //temp stores width
  ptr->module->w = ptr->module->h;    //width becomes height
  ptr->module->h = temp;           //height becomes width
  return;
}
...
// I did this by first checking what the current cutline is, and then swapping
// as necessary.
void recut(node_t* ptr) {
  if(!is_internal_node(ptr)) return;
  assert(ptr->module == NULL && ptr->cutline != UNDEFINED_CUTLINE);
  // TODO:
  if (ptr->cutline == H)  //if horizontal , change to vertical
  {
    ptr->cutline = V;
  }
  else
  {
    ptr->cutline = H;        //if vertical, change to horizontal
  }
  return;
}
...
//I did this by again using a temp node to store the module_t and then swapped.
void swap_module(node_t* a, node_t* b) {
  ...
  node_t* temp = (node_t*)malloc(sizeof(node_t)); //malloc a new node of type node_t
  temp->module = a->module;        // temp contains a->module
  a->module = b->module;          // a swap with b
  b->module = temp->module;      //b swap with previous a module
  return;
  // TODO:
}
...
//I did this by swapping the necessary parent and child pointers.
void swap_topology(node_t* a, node_t* b) {
  ...
  if (a == a->parent->right)
  {
    a->parent->right = b;      //swap child
  }
  else if (a == a->parent->left)
  {
    a->parent->left = b;      //swap child
  }
  if (b == b->parent->right)
  {
    b->parent->right = a;     //swap child
  }
  else if (b == b->parent->left)
  {
    b->parent->left = a;      //swap child
  }

  temp->parent = a->parent;
  a->parent = b->parent;
  b->parent = temp->parent;      //swap parents
  return;
}
...
postfix_traversal(ptr->left,nth,expression);  //recursive call left
postfix_traversal(ptr->right,nth,expression); //recursive call right
if (ptr->cutline == UNDEFINED_CUTLINE)
{
	expression[*nth].module = ptr->module;
	expression[*nth].cutline = UNDEFINED_CUTLINE;  //get module number
	*nth += 1;
	return;
}
else
{
	...
	expression[*nth].cutline = ptr->cutline;   //get cutline
	*nth += 1;
	return;
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  {
	return create_child(par,n);  //makes the last left a leaf node.
  }

  node_t* internal = (node_t*)malloc(sizeof(node_t)); //malloc internal node
  internal->cutline = V;
  internal->module = NULL;
  internal->parent = par;
  internal->right = create_child(internal,n);  //call to make right child
  internal->left = init_slicing_tree(internal, n+1); //left child is recursively created until the last leaf is left.
  return internal;
}
//helper function to create the right child of the internal nodes.
node_t* create_child(node_t* par, int n)
{
	node_t* leaf = (node_t*)malloc(sizeof(node_t));
  leaf->module = (module_t*)malloc(sizeof(module_t)); //making right leaf child
	leaf->module->idx = n;
  leaf->left = NULL;
  leaf->right = NULL;
	leaf->parent = par;
