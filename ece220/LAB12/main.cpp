...
//INSERT YOUR CODE HERE:
	Rectangle add_rect(0,0);
	int i=0;
	while (i < number_of_rectangles)
	{
		add_rect = add_rect + all_rectangles[i];
		i++;
	}
	return add_rect.area();
}
...
int calc_perimeter_of_product_Rectangle(vector<Rectangle> all_rectangles)
{
	int number_of_rectangles = all_rectangles.size();
//INSERT YOUR CODE HERE:
	Rectangle mult_rect(1,1);
	int i=0;
	while (i<number_of_rectangles)
	{
		mult_rect = mult_rect * all_rectangles[i];
		i++;
	}
	return mult_rect.perimeter();
}
